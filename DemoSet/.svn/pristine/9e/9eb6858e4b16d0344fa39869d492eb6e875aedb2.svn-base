...
public class BidirSlidingLayout extends RelativeLayout implements
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed) {
			...
			((RelativeLayout) leftMenuLayout).getChildAt(0)
			...
			((RelativeLayout) rightMenuLayout).getChildAt(0)
			.setOnTouchListener(new OnTouchListener() {
			...
			setScrollEvent(contentLayout);
			((MyLinearLayout) contentLayout).bsl = this;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		// return true;
		// }
		boolean onTouch = onTouch(this, ev);
		System.out.println("layout onInterceptTouchEvent: " + onTouch);
		return onTouch;
	...
	@Override
	public boolean onTouch(View v, MotionEvent event) {
		System.out.println("layout onTouch showMenu: " + showMenu
				+ ", action: " + event.getAction() + ", view: " + v.toString());
		if (!showMenu) {
			return false;
		}
		if (!showMenu && !isLeftMenuVisible && !isRightMenuVisible) {
			return false;
		}
		int moveDistanceX = 0;
		int moveDistanceY = 0;
		...
		case MotionEvent.ACTION_MOVE:
			...
			moveDistanceX = (int) (xMove - xDown);
			moveDistanceY = (int) (yMove - yDown);
			// 检查当前的滑动状态
			checkSlideState(moveDistanceX, moveDistanceY);
			// System.out.println("slideState: " + slideState + ", xMove: "
			// + xMove + ", xDown: " + xDown + ", moveDistanceX: "
			// + moveDistanceX + ", moveDistanceY: " + moveDistanceY
			// + ", touchslap: " + touchSlop);
			// System.out.println("isLeftMenuVisible: " + isLeftMenuVisible
		...
		if (v.isEnabled()) {
			...
			if (0 != slideState && (isLeftMenuVisible || isRightMenuVisible)) {
			...
			// return true;
			// }
			// if (showLeftMenu && moveDistanceX > 0) {
			// System.out.println("return true 3");
			// return true;
			// } else if (showRightMenu && moveDistanceX < 0) {
