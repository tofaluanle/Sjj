 ...
 */
public class CommonsFTPEngine {
	public FTPClient ftpClient = new FTPClient();

	public CommonsFTPEngine() {
		// 设置将过程中使用到的命令输出到控制台
		...
		if (f.exists()) {
			...
			if (localSize >= lRemoteSize) {
				...
				if (!delete) {
					...
					System.out.println("本地文件删除成功，从新下载");
	...
	public long getFileDate(String pathname) {
		try {
			FTPFile[] files = ftpClient.listFiles(new String(pathname
					.getBytes("GBK"), "iso-8859-1"));
			if (0 != files.length)
	...
	public String getFileModifiTime(String pathname) {
		try {
			String modificationTime = ftpClient.getModificationTime(pathname);
	...
	public TimeZone getFileTimeZone(String pathname) {
		try {
			FTPFile[] files = ftpClient.listFiles(new String(pathname
					.getBytes("GBK"), "iso-8859-1"));
			if (0 != files.length)
				return files[0].getTimestamp().getTimeZone();
			return null;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
