...
/**
 * @author 宋疆疆 Displays a video file. The VideoView class can load images from
...
public class VideoView extends SurfaceView implements MediaPlayerControl {
    ...
    /**
     * add by 宋疆疆 监听器，当视频、音频、视频播放前得缓冲
     */
    ...
    /**
     * add by 宋疆疆 自定义监听器，用于监听屏蔽大小是否改变
     */
    ...
    /**
     * add by 宋疆疆 自定义监听器，用于监听屏蔽大小是否改变,用于提供给其他类调用。
     */
     ...
     * 调节音量大小
     *
     * @param index MUSIC的音量从0~150
     */
    public int setVolumeHigh(int index) {
        if (index <= 14) {
            index = index + 1;
        }
        audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, index * 10, 0);
        // Looger.i(
        // APP.VideoView,
        // "audio size: "
        // + audioManager
    ...
    public int getVolume() {
        return audioManager.getStreamVolume(AudioManager.STREAM_MUSIC) / 10;
    }
     ...
     * @param index MUSIC的音量从0~150
     */
    public int setVolumeLow(int index) {
        if (index >= 1) {
            index = index - 1;
        }
        audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, index * 10, 0);
    ...
    public void setVolume(int index) {
        if (index >= 0 && index <= 15) {
            audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, index * 10,
                    0);
        }
    }
    ...
    private MediaPlayer.OnErrorListener mErrorListener = new MediaPlayer.OnErrorListener() {
        public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
            ...
            if (getWindowToken() != null) {
                ...
                // .setPositiveButton(com.android.internal
    ...
    private MediaPlayer.OnBufferingUpdateListener mBufferingUpdateListener = new MediaPlayer.OnBufferingUpdateListener() {
        public void onBufferingUpdate(MediaPlayer mp, int percent) {
            ...
            // LogUtil.i(sTAG, "videoView:缓冲数据为："+String.valueOf(percent));
    ...
    public void start() {
        ...
        // QuitTimeManager.getInstance().setQuitTime(2000,
        // QuitTimeManager.VIDEO_LEVEL);
    }
