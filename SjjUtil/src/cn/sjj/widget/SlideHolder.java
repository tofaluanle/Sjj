 ...
 * 侧栏菜单控件
 * 
 * @author https://github.com/dmitry-zaitsev/AndroidSideMenu
 * @date 2013-11-11 下午3:12:18
 */
public class SlideHolder extends FrameLayout {
	...
	private void initView() {
		mCachedPaint = new Paint(Paint.ANTI_ALIAS_FLAG
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mDirection == DIRECTION_LEFT) {
			menu.layout(parentLeft, parentTop, parentLeft + menuWidth,
					parentBottom);
		} else {
			menu.layout(parentRight - menuWidth, parentTop, parentRight,
					parentBottom);
		}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if (((!mEnabled || !mInterceptTouch) && mMode == MODE_READY)
				|| mAlwaysOpened) {
		...
		if (mMode != MODE_FINISHED) {
			...
			if (!rect.contains((int) ev.getX(), (int) ev.getY())) {
				if (action == MotionEvent.ACTION_UP && mCloseOnRelease
						&& !mDispatchWhenOpened) {
					close();
	...
	private boolean isSlideAllowed() {
		return (mDirection * mEndOffset > 0
				&& mDirection * mOffset < mDirection * mEndOffset && mDirection
				* mOffset >= mDirection * mStartOffset)
				|| (mEndOffset == 0
						&& mDirection * mOffset > mDirection * mEndOffset && mDirection
						* mOffset <= mDirection * mStartOffset);
	}
	...
	private void finishSlide() {
		if (mDirection * mEndOffset > 0) {
			if (mDirection * mOffset > mDirection * mEndOffset / 3) {
			...
			if (mDirection * mOffset < mDirection * mStartOffset / 3 * 2) {
				if (mDirection * mOffset < mDirection * mEndOffset)
					mOffset = mEndOffset;
